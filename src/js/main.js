...
// Fix bfcache (back-forward cache): browser restores scroll before JS runs
...
(function () {
  ...
  if (initialHash) {
    ...
    if (hashTarget) {
      ...
      // Cancel scroll if user switches away before timer fires
      document.addEventListener('visibilitychange', function cancelHashScroll() {
        if (document.visibilityState === 'hidden') {
          clearTimeout(hashTimer);
          document.removeEventListener('visibilitychange', cancelHashScroll);
        }
      });
    }
  }
